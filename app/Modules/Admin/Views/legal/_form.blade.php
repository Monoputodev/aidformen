...
<div class="row">
    ...
    <div class="col-md-6">
        <div class="form-group">
            <div class="form-line">
                {!! Form::label('pdf_link', 'File Link', array('class' => 'col-form-label')) !!} (<span
                  class="error">Supported format :: pdf,doc,docx & file size max :: 2MB</span>)

                <div style="position:relative;">
                    <a class='btn btn-primary btn-sm font-10' href='javascript:;'>
                        Choose File...
                        <input name="pdf_link" type="file"
                ...
                @if(isset($data['pdf_link'] ) && !empty($data['pdf_link']) )
                <a target="_blank" href="{{URL::to('')}}/uploads/legal/{{$data->pdf_link}}" style="margin-top: 5px;"
                  class="btn btn-primary btn-sm font-10"><img src="{{URL::to('')}}/uploads/legal/{{$data->pdf_link}}"
                      height="50px" alt="{{$data['pdf_link']}}">
                </a>

                @endif

                <span class="error">{!! $errors->first('pdf_link') !!}</span>
            </div>
        </div>
    </div>
    <div class="col-md-6">
                ...
                <div style="position:relative;">
                    <a class='btn btn-primary btn-sm font-10' href='javascript:;'>
                        ...
                        <input name="image_link" type="file"
                          style='position:absolute;z-index:2;top:0;left:0;filter: alpha(opacity=0);-ms-filter:"progid:DXImageTransform.Microsoft.Alpha(Opacity=0)";opacity:0;background-color:transparent;color:transparent;'
                          name="file_source" size="40" onchange='$("#upload-image-info").html($(this).val());'>
                    </a>
                    &nbsp;
                    <span class='label label-info' id="upload-file-info"></span>
                </div>
                @if(isset($data['image_link'] ) && !empty($data['image_link']) )
                <a target="_blank" href="{{URL::to('')}}/uploads/legal/{{$data->image_link}}" style="margin-top: 5px;"
                  class="btn btn-primary btn-sm font-10"><img src="{{URL::to('')}}/uploads/legal/{{$data->image_link}}"
