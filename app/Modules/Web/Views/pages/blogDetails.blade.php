...
<div id="fb-root"></div>
<script async defer crossorigin="anonymous" src="https://connect.facebook.net/en_GB/sdk.js#xfbml=1&version=v8.0" nonce="fV5EqVJc"></script>
</head>
<body>
	<!-- LOAD PAGE -->
				...
				<div class="row">
					...
					<div class="col-sm-12 col-md-8">
						<div class="single-news">
							<div class="image">
								<img src="{{ URL::to('uploads/blog') }}/orginal_image/{{ $data->image_link }}" alt="{{ $data->title }}" class="img-fluid"> 
							</div>
							<h2 class="blok-title">
								{{ $data->title }}
							</h2>
							<p>By: <span style="color: #ff7000">{{$data->author}}</span> || Date: <span style="color: blue"> {{date('d M y', strtotime($data->date))}}</span></p>
							<div class="social-icons-div social-icons-bottom">

							<!-- Go to www.addthis.com/dashboard to customize your tools -->
<script type="text/javascript" src="//s7.addthis.com/js/300/addthis_widget.js#pubid=ra-5f93f226848ce5ac"></script>


								<div class="addthis_inline_share_toolbox" data-url="https://aidformen.com<?php  print $_SERVER['REQUEST_URI']; ?>" data-title="{{$data->title}}" data-description="{{$data->description}}" data-media="{{ URL::to('uploads/blog') }}/orginal_image/{{ $data->image_link }}"></div>
							</div>
							{!! $data->description !!}
					...
					@foreach($blogmore as $values)
						<div class="recent_blog_list">
							<a href="{{ route('blog.details',[$values->id,$values->title]) }}">
								<img src="{{ URL::to('uploads/blog') }}/200x200/{{ $values->image_link }}" alt=""><p>{{ $values->title }}</p>
							</a>
